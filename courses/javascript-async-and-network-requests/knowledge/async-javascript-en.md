...
Async codeallows a program to start a long-running task (like fetching data from a file).
and continue with other tasks before the first one finishes.

Async codeprevents the application from freezing, which is critical for user experience.

## Control Flow

Control Flowis the order in which statements are executed in a program.

By default, JavaScript runs codefrom top to bottomand left to right.
...
The examples above are normalsynchronous flow.

## Why Async Code

Some tasks take time to finish(network requests, timers, user input).

To stay responsive, JavaScript can useasync programming.

Asynchronous flowrefers to how JavaScript allows certain operations torun in the backgroundand let their results behandled when they are ready.
...
- Timersrun after a specified number of milliseconds
- Eventsrun when triggered by an event
- Network requestsrun when the data arrives
...
Result isundefinedbecause the async code has not finished yet.

Beginners expect async results immediately.

## JavaScript Events

Eventsare actions or occurrences that happen in the browser,
...
A single-threaded JavaScript engine handles asynchronous tasks by using an event
loop to switch between them, rather than utilizing multiple CPU cores.
When a task finishes, it signals the main thread (via a callback, promise, or event) to handle the result.

In short, asynchronous tells the system:

1. Start this task now
1. I don't need the result immediately
1. Notify me later when it's done
...
1. Sit down and do other things while the chef makes it
1. The server brings the food (callback)
...
- What areTimeouts
- Whycallbackswere created
- Howpromisesrepresent future values.
- Whyasyncandawaitare preferred.
- How todebugasync code.
