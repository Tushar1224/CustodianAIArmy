...
export default function HomePage() {
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', color: 'var(--text)', fontFamily: "'Exo 2', sans-serif" }}>
      ...
      {/* ─── Hero Banner ─── */}
      <section style={{ padding: '6rem 2rem 2rem', background: 'linear-gradient(180deg, var(--bg3) 0%, var(--bg2) 50%, var(--bg) 100%)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '3rem', flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 400px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'var(--primary-glow)', border: '1px solid var(--primary)', color: 'var(--primary)', padding: '0.35rem 1rem', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 600, marginBottom: '1rem' }}>
            ...
            <h1 style={{ fontFamily: "'Orbitron', monospace", fontSize: 'clamp(2rem, 4vw, 3.2rem)', fontWeight: 900, lineHeight: 1.1, margin: '0 0 1rem', letterSpacing: '1px' }}>
              <span style={{ color: 'var(--primary)' }}>Custodian AI</span><br />
              <span style={{ color: 'var(--text)' }}>Army</span>
            </h1>
            <p style={{ color: 'var(--text2)', fontSize: '1.1rem', maxWidth: '500px', lineHeight: 1.6, marginBottom: '2rem' }}>
          ...
          <div style={{ flex: '0 0 280px', textAlign: 'center' }}>
            <div style={{ width: '260px', height: '260px', background: 'linear-gradient(135deg, var(--primary-glow), rgba(124,58,237,0.08))', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--border-hover)', position: 'relative' }}>
              <i className="fas fa-robot" style={{ fontSize: '6rem', color: 'var(--primary)', opacity: 0.8 }}></i>
              <div style={{ position: 'absolute', width: '100%', height: '100%', borderRadius: '50%', border: '1px solid var(--border)', animation: 'pulseRing 3s ease-in-out infinite' }}></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '4rem 2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h2 style={{ fontFamily: "'Orbitron', monospace", fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', fontWeight: 900, color: 'var(--text)', marginBottom: '0.5rem' }}>
            Everything You Need
          </h2>
          <div style={{ width: '50px', height: '3px', background: 'var(--primary)', margin: '0.75rem auto', borderRadius: '2px' }}></div>
          <p style={{ color: 'var(--text2)', fontSize: '1rem', maxWidth: '500px', margin: '0 auto' }}>Seven powerful tools to supercharge your learning, building, and career journey.</p>
        </div>
        <div className="features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem', maxWidth: '1100px', margin: '0 auto' }}>
          {allFeatures.map((f, i) => (
            <div
              ...
              style={{
                background: 'var(--card)',
                border: `1px solid ${f.status === 'working' ? 'var(--border)' : 'rgba(245,158,11,0.2)'}`,
                borderRadius: '16px',
                padding: '2rem',
              ...
              onMouseLeave={(e) => {
                e.currentTarget.style.borderColor = f.status === 'working' ? 'var(--border)' : 'rgba(245,158,11,0.2)';
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <div>
                  <h3 style={{ fontFamily: "'Orbitron', monospace", fontSize: '1rem', color: 'var(--text)', margin: '0 0 0.25rem' }}>
                    {f.name}
                  </h3>
                  {f.status === 'working' ? (
              ...
              <p style={{ color: 'var(--text2)', fontSize: '0.9rem', lineHeight: 1.6, margin: '0 0 1rem', flex: 1 }}>
                {f.description}
              </p>
              {f.status === 'working' && (
      ...
      <section style={{ padding: '4rem 2rem', background: 'var(--bg2)' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <div style={{ width: '50px', height: '3px', background: 'var(--primary)', margin: '0.75rem auto', borderRadius: '2px' }}></div>
          <p style={{ color: 'var(--text2)', fontSize: '0.95rem', maxWidth: '500px', margin: '0 auto' }}>Get started in seconds. No complex setup required.</p>
        </div>
        <div className="steps-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2rem', maxWidth: '900px', margin: '0 auto' }}>
          {[
            ...
            <div key={i} className="step-card" style={{ textAlign: 'center', padding: '2rem 1rem' }}>
              ...
              <p style={{ color: 'var(--text2)', fontSize: '0.9rem', margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '4rem 2rem' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          ...
          <div style={{ width: '50px', height: '3px', background: 'var(--primary)', margin: '0.75rem auto', borderRadius: '2px' }}></div>
          <p style={{ color: 'var(--text2)', fontSize: '0.95rem', maxWidth: '500px', margin: '0 auto' }}>Start free. Upgrade when you need more power.</p>
        </div>
        <div className="pricing-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '2rem', maxWidth: '800px', margin: '0 auto' }}>
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '16px', padding: '2.5rem 2rem', textAlign: 'center', boxShadow: 'var(--shadow)' }}>
            <div style={{ fontFamily: "'Orbitron', monospace", fontSize: '1.1rem', color: 'var(--text2)', marginBottom: '0.5rem' }}>Free</div>
            <div style={{ fontFamily: "'Orbitron', monospace", fontSize: '3rem', fontWeight: 900, color: 'var(--primary)', marginBottom: '0.25rem' }}>
              $0 <span style={{ fontSize: '1rem', color: 'var(--text3)' }}>/mo</span>
            </div>
            <div style={{ color: 'var(--text3)', fontSize: '0.875rem', marginBottom: '2rem' }}>Perfect for trying out the platform</div>
            <ul style={{ listStyle: 'none', textAlign: 'left', marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: 0 }}>
              {['3 requests/day as guest', '20 requests/day (signed in)', 'All AI agents', 'Temporary chat history (guest)', 'Permanent chat history (signed in)', 'Gemini + Claude (signed in)', 'Learn with AI courses', 'Custom Agents Studio'].map((f, i) => (
                <li key={i} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.9rem', color: 'var(--text2)' }}>
                  <i className="fas fa-check" style={{ color: 'var(--success)', width: '16px' }}></i> {f}
                </li>
              ))}
            </ul>
            <button onClick={() => navigate('/dashboard')} style={{ width: '100%', padding: '0.75rem', borderRadius: '10px', fontWeight: 700, fontSize: '0.95rem', cursor: 'pointer', background: 'transparent', color: 'var(--primary)', border: '2px solid var(--primary)', transition: 'all 0.2s' }}
              onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--primary-glow)'; }}
              onMouseLeave={(e) => { e.currentTarget.style.background = 'transparent'; }}>
              Get Started Free
            </button>
          ...
          <div style={{ background: 'var(--card)', border: '2px solid var(--primary)', borderRadius: '16px', padding: '2.5rem 2rem', textAlign: 'center', position: 'relative', boxShadow: 'var(--shadow-hover)' }}>
            ...
            <div style={{ fontFamily: "'Orbitron', monospace", fontSize: '3rem', fontWeight: 900, color: 'var(--primary)', marginBottom: '0.25rem' }}>
              $9 <span style={{ fontSize: '1rem', color: 'var(--text3)' }}>/mo</span>
            </div>
            <div style={{ color: 'var(--text3)', fontSize: '0.875rem', marginBottom: '2rem' }}>For power users and professionals</div>
            <ul style={{ listStyle: 'none', textAlign: 'left', marginBottom: '2rem', display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: 0 }}>
      ...
      <section style={{ padding: '4rem 2rem', background: 'var(--bg2)' }}>
        <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
          <h2 style={{ fontFamily: "'Orbitron', monospace", fontSize: 'clamp(1.5rem, 3vw, 2.2rem)', fontWeight: 900, color: 'var(--text)', marginBottom: '0.5rem' }}>
            What's <span style={{ color: 'var(--primary)' }}>Coming Next</span>
          </h2>
          <div style={{ width: '50px', height: '3px', background: 'var(--primary)', margin: '0.75rem auto', borderRadius: '2px' }}></div>
